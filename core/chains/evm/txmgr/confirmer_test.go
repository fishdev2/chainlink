...
func TestEthConfirmer_Lifecycle(t *testing.T) {
	...
	feeEstimator := gas.NewWrappedEvmEstimator(estimator, config.EVM().GasEstimator().EIP1559DynamicFees())
	txBuilder := txmgr.NewEvmTxAttemptBuilder(*ethClient.ConfiguredChainID(), config, config.EVM().GasEstimator(), ethKeyStore, feeEstimator)
	ec := txmgr.NewEvmConfirmer(txStore, txmgr.NewEvmTxmClient(ethClient), txmgr.NewEvmTxmConfig(config), txmgr.NewEvmTxmFeeConfig(config.EVM().GasEstimator()), config.EVM().Transactions(), config.Database(), ethKeyStore, txBuilder, lggr)
...
func TestEthConfirmer_RebroadcastWhereNecessary_WithConnectivityCheck(t *testing.T) {
	...
	t.Run("should retry previous attempt if connectivity check failed for legacy transactions", func(t *testing.T) {
		...
		ec := txmgr.NewEvmConfirmer(txStore, txmgr.NewEvmTxmClient(ethClient), evmcfg, txmgr.NewEvmTxmFeeConfig(ccfg.EVM().GasEstimator()), ccfg.EVM().Transactions(), cfg.Database(), kst, txBuilder, lggr)

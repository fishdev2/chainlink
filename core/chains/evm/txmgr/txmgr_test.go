...
func makeTestEvmTxm(
	t *testing.T, db *sqlx.DB, ethClient evmclient.Client, cfg txmgr.Config, gcfg txmgr.FeeConfig, estimator gas.EvmFeeEstimator, txConfig evmconfig.Transactions, dbConfig txmgr.DatabaseConfig, listenerConfig txmgr.ListenerConfig, keyStore keystore.Eth, eventBroadcaster pg.EventBroadcaster) (txmgr.EvmTxManager, error) {
...
func TestTxm_SendNativeToken_DoesNotSendToZero(t *testing.T) {
	...
	txm, err := makeTestEvmTxm(t, db, ethClient, config, evmConfig.GasEstimator(), estimator, evmConfig.Transactions(), dbConfig, dbConfig.Listener(), keyStore, nil)
...
func TestTxm_CreateTransaction(t *testing.T) {
	...
	estimator := gas.NewEstimator(logger.TestLogger(t), ethClient, config, evmConfig.GasEstimator())
	txm, err := makeTestEvmTxm(t, db, ethClient, config, evmConfig.GasEstimator(), estimator, evmConfig.Transactions(), dbConfig, dbConfig.Listener(), kst.Eth(), nil)
	require.NoError(t, err)
...
func TestTxm_Lifecycle(t *testing.T) {
	...
	estimator := gas.NewEstimator(logger.TestLogger(t), ethClient, config, evmConfig.GasEstimator())
	txm, err := makeTestEvmTxm(t, db, ethClient, config, evmConfig.GasEstimator(), estimator, evmConfig.Transactions(), dbConfig, dbConfig.Listener(), kst, eventBroadcaster)
	require.NoError(t, err)
...
func TestTxm_Reset(t *testing.T) {
	...
	txm, err := makeTestEvmTxm(t, db, ethClient, cfg, cfg.EVM().GasEstimator(), estimator, cfg.EVM().Transactions(), cfg.Database(), cfg.Database().Listener(), kst.Eth(), eventBroadcaster)
	require.NoError(t, err)
